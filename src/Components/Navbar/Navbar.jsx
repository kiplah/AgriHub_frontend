...
const Navbar = ({ bground }) => {
  ...
  const [showInputsDropdown, setShowInputsDropdown] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (inputsDropdownRef.current && !inputsDropdownRef.current.contains(event.target)) {
        setShowInputsDropdown(false);
      }
    };

    if (showInputsDropdown) {
      document.addEventListener("mousedown", handleClickOutside);
    }
  ...
  return (
    <div
      className={`${bg ? "bg-white shadow-lg" : "bg-transparent"
    ...
    >
      ...
      <div className="mx-auto w-[80%] relative">
        <div className="flex flex-col md:flex-row gap-5 md:gap-0 justify-between items-center py-4">
          ...
          >
            <span
              className={`block w-[30px] h-1 ${bg ? "bg-black" : "bg-white"
                } mb-1 rounded-full`}
            ></span>
            <span
              className={`block w-[30px] h-1 ${bg ? "bg-black" : "bg-white"
        ...
        >
          <div
            className={`${showNav ? "block absolute right-[30px] top-[30px]" : "hidden"
              }`}
            onClick={handleNavbar}
          >
          ...
          <ul
            className={`${showNav ? "block" : "hidden"
              } w-full md:flex  flex-col md:flex-row justify-around  mx-auto ${bg ? "text-black gap-10 " : "text-[#017d29] md:text-white "
              }`}
          >
            <Link href="/">

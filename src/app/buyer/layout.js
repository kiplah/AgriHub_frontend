...
export default function Layout({ children, initialCollapsed = false }) {
  ...
  const userName = user?.username || user?.name || user?.email?.split("@")[0] || "Buyer";
  // Create initials safely
  const userInitials = (userName || "Buyer")
    .split(" ")
    .map((n) => n[0])
    .join("")
    .toUpperCase()
  ...
  const menu = [
    ...
    { key: "messages", label: "Messages", href: "/buyer/chats", icon: <MessageSquare className="w-5 h-5" /> },
    { key: "settings", label: "Settings", href: "/buyer/settings", icon: <Settings className="w-5 h-5" /> },
  ];
        ...
        <div className="px-3 py-4 border-t">
          <div className={`flex items-center gap-3 ${collapsed ? "md:justify-center" : ""}`}>
            ...
            <div className={`${collapsed ? "md:hidden" : "block"}`}>
              <div className="text-sm font-semibold text-gray-900 truncate max-w-[120px]">{userName}</div>
          ...
          <div className={`mt-4 ${collapsed ? "md:hidden" : "block"}`}>
            ...
            <Link href="/buyer/support" className="block w-full mt-2">
              <button className="w-full py-2 px-3 rounded-md border border-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-900 hover:text-white transition">Get Help</button>
            </Link>
          </div>
        </div>
      </aside>
        ...
        <div className="md:hidden w-full bg-white border-b sticky top-0 z-30">
          ...
          {profileOpen && (
            <div className="absolute right-2 top-14 w-48 bg-white border rounded-lg shadow-lg py-1 z-50">
              ...
              <Link href="/buyer/settings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setProfileOpen(false)}>My Profile</Link>
              <Link href="/buyer/settings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setProfileOpen(false)}>Settings</Link>
              ...
              >
                <div className="text-right hidden sm:block">
                  <div className="text-sm font-medium text-gray-900 flex items-center justify-end gap-2">
                    {showBalance ? `Spent KES ${totalSpent.toLocaleString()}` : "••••••"}
                    <button
                  ...
                  <div className="text-xs text-emerald-600 font-medium">Verified Buyer</div>
                </div>
